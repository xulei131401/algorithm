package main

import (
	"log"
)

/*
*
两路归并排序（Merge Sort）是建立在归并操作上的一种有效，稳定的排序算法，
该算法是采用分治法（Divide and Conquer）的一个非常典型的应用。将已有序的子序列合并，
得到完全有序的序列；即先使每个子序列有序，再使子序列段间有序。
若将两个有序表合并成一个有序表，称为二路归并。
时间复杂度：O(n log n)
空间复杂度：T（n)
稳定性：稳定


步骤：
	1.递归拆分元素
	2.底层两两有序合并，拆分到最后只剩一个元素，自身有序


*/
func main() {
	nums := []int{8, 4, 5, 7, 1, 3, 6, 2, 10, 20, 17, 25, 24, 28}
	log.Println("归并排序-递归:", sortArray(nums))
	log.Println("归并排序-栈:", sortArray2(nums))
}

func sortArray(nums []int) []int {
	mergeSort1(nums, 0, len(nums)-1)
	return nums
}

// mergeSort 递归实现
// 递归实现先分组，然后自顶向下
func mergeSort1(nums []int, left, right int) {
	// 拆分的过程可以使用迭代方式优化
	if left < right {
		mid := left + (right-left)>>1 // 拆分点
		// 递归过程的2次mergeSort1调用可以在迭代过程中去掉优化
		mergeSort1(nums, left, mid)
		mergeSort1(nums, mid+1, right)
		// 拆分完毕之后再重组
		merge1(nums, left, mid, right)
	}
}

// merge1 这个函数的作用相当于是 left~mid,mid+1~right两个区间的数组有序合并到一个新的数组中
func merge1(nums []int, left, mid, right int) {
	arr := make([]int, right-left+1)
	i, j, k := left, mid+1, 0 // k代表最新的结果的位置
	for i <= mid && j <= right {
		if nums[i] < nums[j] {
			arr[k] = nums[i]
			i++
		} else {
			arr[k] = nums[j]
			j++
		}

		k++
	}

	// 检查是左边是否有剩余元素
	for i <= mid {
		arr[k] = nums[i]
		k++
		i++
	}

	// 检查是右边边是否有剩余元素
	for j <= right {
		arr[k] = nums[j]
		k++
		j++
	}

	// 重新赋值nums
	for p := 0; p < len(arr); p++ {
		// 原来的数组从left开始
		nums[left+p] = arr[p]
	}
}

// sortArray 迭代实现
// 迭代过程是先两两排序分组，最后四个四个排序分组，直至完成排序，从底向上
func sortArray2(nums []int) []int {
	step := 1 // 步长间隔为1
	length := len(nums)

	for step < length {
		// 每次从头开始合并，每次步长*2
		for i := 0; i+step < length; i += step * 2 {
			left := i
			right := i + step*2 - 1
			mid := i + step - 1

			// 右边界不能超
			if right > length-1 {
				right = length - 1 //整个待排序数组为奇数的情况
			}

			merge1(nums, left, mid, right)
		}

		step *= 2
	}

	return nums
}
