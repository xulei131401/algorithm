...
func (h ArrayHeap) parentIndex(i int) int {
	return (i - 1) / 2
}

func (h ArrayHeap) leftChildIndex(i int) int {
	// 使用这个值的时候要注意，有可能超出len(h)
	return 2*i + 1
}

func (h ArrayHeap) rightChildIndex(i int) int {
	// 使用这个值的时候要注意，有可能超出len(h)
	return 2*i + 2
}
...
func (h *ArrayHeap) Remove(i int) (int, bool) {
	// 只能删除数组内的元素
	if i < 0 || i > len(*h)-1 {
		return 0, false
	}
